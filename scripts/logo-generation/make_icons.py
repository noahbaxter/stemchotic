...
Run after render_logo.py. Produces:
  packaging/macos/Stemchotic.icns   (iconutil, macOS-only step)
  packaging/windows/stemchotic.ico  (multi-resolution)
  packaging/linux/stemchotic.png    (256x256)
"""
...
ROOT = HERE.parents[1]
MASTER = HERE / "stemchotic_logo.png"


def main():
    master = Image.open(MASTER).convert("RGBA")

    linux = ROOT / "packaging" / "linux" / "stemchotic.png"
    linux.parent.mkdir(parents=True, exist_ok=True)
    master.resize((256, 256), Image.LANCZOS).save(linux)

    win = ROOT / "packaging" / "windows" / "stemchotic.ico"
